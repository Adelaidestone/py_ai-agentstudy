当单靠提示词工程无法满足需求时，就需要考虑进一步的工程手段，本篇围绕五大模块的后四项展开：RAG、微调、续训、智能体开发。虽然他们表面都在“增强模型能力”，但是实际解决的问题并不一样
# 1.问题诊断
真实项目中，ai应用回答不好时，最容易犯的错误是：还没有判断问题根因，就直接说要不要微调，上不上Agent
先问四个问题：
先问四个问题会稳很多：
...
![从提示词、RAG、微调、续训到智能体的方案诊断关系图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-1-1-1.png?lastModify=1783408199)
...
# 2.RAG
RAG（Retrieval-Augmented Generation,检索增强生成）是一种把信息检索和大模型生成结合起来的应用框架
基本流程：
1.用户提问
2.系统去外部知识库找相关的资料
3.把“问题+检索结果”发给大模型
4.由模型基于这些资料生成答案
RAG的重点不是“修改模型参数”，而是在运行时给模型补充他当前需要的上下文
记住这个最短定义：RAG=先查资料，再回答问题
### 2.1 为什么 RAG 往往是项目里的第一选择
...
## 2.3 RAG的具体场景
当你遇到如下场景，优先验证RAG通常比较稳妥：
- 需要接入企业内部资料、产品文档、FAQ、制度手册、接口说明等私有知识
- 需要回答最新信息、时效性很强的问题
- 希望回答结果附带依据和来源
- 希望在不改模型参数情况下，快速迭代知识内容
典型场景包括：
- 企业知识库问答
- 智能客服和售后助手
- 研发文档问答
...
## 3、微调（Fine-tuning）
微调(Fine-tuning)是指在已经训练的基础模型上，继续用特定数据做训练，使得模型在某类任务、某种风格和或者某种输出形式上表现更稳定
在工程语境中，可以理解为：**微调是在改模型的“行为方式”，而不是在给模型外挂一套知识库**
他最常见的目标包括：
- 让模型更稳定地遵循指令
- 让模型输出固定格式，例如JSON、SQL、标签、分类结果
-  让模型更符合某种业务风格、品牌口吻、术语习惯
- 让模型在某类任务上更稳，例如分类、抽取、代码补全、客服回复
需要特别提醒的是，微调虽然可能让模型记住一些样例，但是他不适合承载高频变化的知识。如果知识经常变，仍然需要优先考虑RAG和外部知识库
...
# 5.智能体开发
## 5.1 什么是智能体
在经典的ai语境中，智能体指能够感知环境、做出决策并执行动作，以实现目标的系统。
放到大模型应用开发中，更工程化的定义是：
	智能体 =大模型+工具+状态+执行流程
...
- **短期记忆**：主要是当前对话上下文，受上下文窗口限制。
    ...
    ![短期记忆在对话上下文中的示意图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/3/3-1-5-3.png?lastModify=1783428556)
...
![工具使用扩展大模型能力边界的示意图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/3/3-1-5-4.png?lastModify=1783428556)
...
![规划与任务拆解的示意图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/3/3-1-5-5.png?lastModify=1783428556)

![规划与反思机制的示意图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/3/3-1-5-6.png?lastModify=1783428556)
...
![Function Calling 的基本流程：模型决定工具和参数，程序负责真正执行](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-5-3-1.jpg?lastModify=1783408199)

![Function Calling 在实际接口调用中的工具声明与参数返回示意图](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-5-3-2.jpg?lastModify=1783408199)
...
![MCP 像 AI 世界的 USB-C 接口，用统一协议暴露工具、资源和提示模板](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-5-3-3.jpg?lastModify=1783408199)
...
![MCP 的调用流程：主机、客户端和服务器之间如何完成能力发现与调用](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-5-3-4.jpg?lastModify=1783408199)
...
- **什么时候调？** 因为客户端会把当前所有可用工具及其描述发给模型，模型根据用户问题和工具描述，判断是否需要调用。
    
- **调哪一个？** 因为每个工具都有 `name`、`description`、`parameters`，模型会根据用户问题和工具描述进行匹配。
...
![工作流更强调固定步骤、可控执行和明确顺序的流程编排方式](file:///E:/ai%20agent%20study/ai-agents-from-zero/images/1-3/1-3-5-5-1.jpg?lastModify=1783408199)

例如：先分类、再检索、再生成、再校验、再输出。

这类场景不一定需要 Agent 临场自由决策，更适合工作流。

比如：讯飞星辰 Agent 平台：[https://agent.xfyun.cn/home](https://agent.xfyun.cn/home)
...
--header 'Authorization: Bearer {app-0XDstBbpEyvWwwlvP3T6iRMA}' \
