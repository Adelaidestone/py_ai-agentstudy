...
import com.fletcher.helloai.service.ModelRouterService;
...
public class ChatController {
    ...
    /**
     * MiMo 同步对话
     * 访问: http://localhost:8080/chat/mimo?q=你好
     */
    @GetMapping("/chat/mimo")
    public String mimoChat(@RequestParam("q") String question) {
        return chatService.mimoChat(question);
    }

    /**
     * MiMo 流式对话（SSE）
     * curl -N 'http://localhost:8080/chat/mimo/stream?q=写一首关于小米的诗'
     */
    @GetMapping(value = "/chat/mimo/stream", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<String> mimoChatStream(@RequestParam("q") String question) {
        return chatService.mimoChatStream(question);
    }
    ...
    public String auto(@RequestParam String q) {
        return modelRouterService.routeByModel(q);
    }
